...
import { BulkRemoveSupplier } from "../../types/BulkRemoveSupplier";
 import {
  deleteWarehouseAddUrl,
   deleteWarehouseListUrl,
   DeleteWarehouseListUrlDialog,
...
interface DeleteWarehouseListProps {
  params: DeleteWarehouseListUrlQueryParams;
}
export const DeleteWarehouseListViewComponent: React.FC<DeleteWarehouseListProps> = ({ params }) => {
  ...
  const { data, loading, refetch } = usesDeleteWarehousesListListQuery({
    displayLoader: true,
  ...
  const [
    ...
    cleanupFn: reset,
    createUrl: deleteWarehouseListUrl,
    getFilterQueryParam,
    navigate,
    params
  });
 // todo: put it here console.log(data);
  const [openModal, closeModal] = createDialogActionHandlers<
    DeleteWarehouseListUrlDialog,
    DeleteWarehouseListUrlQueryParams
    >(navigate, deleteWarehouseListUrl, params);
  ...
  const { loadNextPage, loadPreviousPage, pageInfo } = paginate(
    maybe(() => data.requestsDeleteWarehouse.pageInfo),
    paginationState,
    params
  );
  ...
  const handleSort = createSortHandler(navigate, deleteWarehouseListUrl, params);
       ...
       {(BulkRemoveSupplier, bulkRemoveSuppliersOpts) => (
         <>
           <ExistProductsListPage
             ...
             Requests={maybe(() => data.requestsDeleteWarehouse.edges.map(edge => edge.node))}
             settings={settings}
             disabled={loading}
             pageInfo={pageInfo}
             onAdd={() => navigate(deleteWarehouseAddUrl)}
...
export default DeleteWarehouseListViewComponent;
